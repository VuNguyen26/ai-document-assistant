...
export async function getAudioVersions(params?: {
  page?: number;
  limit?: number;
  documentId?: string;
  sourceType?: 'DOCUMENT' | 'SUMMARY';
}): Promise<AudioVersionsListResponse> {
  ...
  const result = await apiFetch<
    AudioVersionsListResponse | ApiEnvelope<AudioVersionsListResponse>
  >(`/audio?${searchParams.toString()}`, {
    method: 'GET',
  });
...
export async function createAudioVersion(
  payload: CreateAudioVersionPayload,
): Promise<AudioVersionItem> {
  const result = await apiFetch<AudioVersionItem | ApiEnvelope<AudioVersionItem>>(
    '/audio/generate',
    {
      method: 'POST',
...
export async function getAudioFileBlob(audioId: string): Promise<Blob> {
  let response = await rawFetch(`/audio/${audioId}/file`, {
    method: 'GET',
  });
